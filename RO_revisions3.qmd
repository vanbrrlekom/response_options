...
Moreover, participants categorize faces categorically [@campanellaCategoricalPerceptionFacial2001]. This phenomenon has been observed when participants categorize faces that have been morphed to vary from feminine to masculine. Categorizations of these morphed faces were accentuated so that for example a 60% female morph was rated as a woman by close to 80% of participants  [@campanellaCategoricalPerceptionFacial2001]. Observing categorical effects for any stimuli suggests that people treat that stimuli as two separate categories [@simanovaLinguisticPriorsShape2016]. The observation of a categorical effect for gender suggests that people think of gender as a strict binary consisting of women and men only. 
...
All participants completed the experiment and were included in the analyses. The one-dimension condition included `r sum(subs$condition == "sd")` participants and the two-dimension condition included `r sum(subs$condition == "md")` participants.
...
```{r descriptives-two, fig.cap= "Participant level and mean ratings of faces in One-dimensiona and two-dimensional conditions"}
...
```{r descriptives-nbo, message=FALSE, error=FALSE, warning=FALSE}
library(ggpubr)
...
```{r desc-1, fig.cap= "Gender Categorizations by Participants"}
