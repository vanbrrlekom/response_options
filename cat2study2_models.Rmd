...
fit_binary_stair <- brm(f_cat ~ 0+ masc*condition + (1 + masc|id) + (1|face), family = bernoulli(link = 'logit'), 
...
fit_binary_index <- brm(f_cat ~ 0 + condition + masc:condition + (1 +masc|id) + (1|face), family = bernoulli(link = 'logit'), 
          prior = c(prior(normal(0,5), class = "b", coef = "conditionmc"),
                    prior(normal(0,3), class ="b", coef= "conditionmc:masc"),
                    prior(normal(0,5), class = "b", coef = "conditionxb"),
                    prior(normal(0,3), class = "b", coef = "conditionxb:masc"),
                    prior(normal(0,5), class = "b", coef = "conditionft"),
...
hypothesis(fit_binary_index, "conditionft= conditionmc" )
hypothesis(fit_binary_index, "conditionft= conditionxb" )


summary(fit_binary_stair)
conditional_effects(fit_binary_stair)

hypothesis(fit_binary_stair, "masc:conditionmc = masc:conditionxb")
```
