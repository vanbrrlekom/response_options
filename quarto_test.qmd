...
*I don't know responses* represented trials where participants did not categorize any gender category. This was computed by dichotomizing the variables so "I don't know" = 1 and all other responses = 0. In the free text condition, participants responses wre manually coded and variations of "unsure" and "I don't know" counted as "I don't know".
...
tmp %>% group_by(id, condition) %>% 
  ...
  geom_bar( #Count the dk categorizations in each condition
               width = 0.6) +
  facet_wrap(~condition)+
  scale_x_continuous(name = "Categorizations")+
...
Most faces were categorized as women or men by most participants (see Figure @fig-desc-1). @fig-desc-1 illustrates the the proportions of categorization (y axis) at each level of facial femininity (x-axis) across the three experimental conditions (separate plots). Participants did categorize faces outside of this binary in the multiple categories condition, as @fig-desc-1 shows, and most such categorizations were made in response to androgynous faces.
