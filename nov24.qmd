...
abstract: "Psychological research often treats gender as a binary and unidimensional even though this oversimplifies the variability of gender. In two experiments, we studied how alternative response options influence how people perceive the gender of a racially diverse set of morphed faces. Experiment 1 (N = 71), compared one-dimensional and two-dimensional scales for gender categorization. Results indicated that participants consistently used the ends of dimensional scales, resulting in highly categorical perceptions. Experiment 2 (N = 105), compared traditional binary response options with multiple categories and free-text answers. Results indicated that providing more response options than the binary (e.g., “non-binary”, “I don’t know”) made participant perceive gender as more diverse, whereas free-text options did not change a binary categorical perception of gender. Despite the opportunity to categorize faces beyond the binary, the predominant categorizations remained as ‘woman’ or ‘man’. We conclude that multiple response options seems to be the best way to increase perceivers understanding of gender beyond the binary. "
...
Surveys and questionnaires that collect data on gender have typically used response formats that do not account for this gender diversity, for example by including only the categories woman/female and male/man as response options [@sapersteinCategoricalGradationalAlternative2021]. This is beginning to change; it is increasingly common for studies to include response formats that are sensitive to a wide variety of gender identities, at least when collecting data on participants' own gender identities [@hydeFutureSexGender2019]. Research on gender categorization of others, however, is still dominated by binary response options [e.g., @campanellaCategoricalPerceptionFacial2001; @habibiSpontaneousGenderCategorization2012; @jungAutomaticityGenderCategorization2019]. In this study, we aim to challenge this trend and test how response formats influence participants' gender categorizations.  


## Gender and Gender Categorization

[what is gender] To understand why response formats matter, it is important to understand the concept of gender itself. Gender is a complicated and multifaceted concept with both personal and social aspects [@hydeFutureSexGender2019]. The personal aspects of gender include a person's physical body, how they choose to present themselves as well as their internal sense of gender (i.e. their gender identity). The social aspects of gender include beliefs about which gender categories exist, prescriptions and proscriptions for appropriate behavior for different gender categories, as well as the language available to refer to gender, including gendered pronouns and the names of gender categories (Lonergan & Palomares, 2020). The personal and social facets of gender are interconnected [@morgenrothEffectsGenderTrouble2021]; social norms constrain the personal aspects of gender and, inversely, personal enactments of gender may influence the social norms over time [@butler1999]. For example, Western societies have long constructed gender as a binary, with language and media rendering transgender and non-binary gender invisible, limiting gender diverse people's ability to know and express their identities [@richardsNonbinaryGenderqueerGenders2016a]. Inversely, as gender diverse people become increasingly visible, norms and language terms are changing, and the wider conceptions of gender change to incorporate transgender and non-binary gender categories [@monroNonbinaryGenderqueerOverview2019, fasoli], though not without resistance (Vergoossen, Morgenroth, Tee & Hegarty)

[gender categorization - intro] Gender categorization is the act of ascribing gender categories to others and is influenced by both the personal and social aspects of gender (Kessler & McKenna, 1978). For example, people base their gender categorizations on other's external features (Yamaguchi et al, 2013, Cloutier et al., 2014), and which features are associated with which gender identities is partially a result of norms (Butler, 1999). 

The processes that shape gender categorization are an important object of study because gender categorization may activate stereotypes and influence judgements of other traits and characteristics, such as aggression (Freeman & Anbady, 2011). Moreover, many studies suggest that gender categorization is a central and important feature of social interactions (Kessler & McKenna, 1978, Liberman et al., 2017); Gender categorization is easy (Yang & Dunham, 2019), fast (Tomilleri & Castelli, 2012), and does not require much visual input (ref). Some have gone so far asto conclude that gender categorization is an automatic (ref) and unavoidable feature of social interactions (Fiske, 1998). 

[categorical perception] Additionally, it has been argued that gender is perceived categorically [@campanellaCategoricalPerceptionFacial2001]. This is supported by research showing that when people categorize faces that vary on a continuum feminine to masculine, their categorizations are exaggerated compared to the level of facial gender (Rule et al., 2012; Campanella et al., 2001; Atwood et al., 2024). For example, a 60% female face was categorized as a woman by closer to 80% of participants [@campanellaCategoricalPerceptionFacial2001]. Such categorical perception effects have been observed for other domains, such as colors (blue, green) and phonemes ("ga", "ba") and suggest that people view the stimuli as consisting of two clear groups or categories [@simanovaLinguisticPriorsShape2016]. The observation of  categorical perception of gender, therefore, implies that people treat gender as a binary consisting of women and men only.

## Gender categorization beyond the binary

[Binary gender categorization is not inevitable] Several recent studies suggest that this binary view of gender may be learnt. For example, in one study, participants were tasked with keeping track of three people carrying out a conversation [Gallagher et al., 2024]. On both implicit and explicit indices of gender categorization, transgender status, age and views on gender predicted participants' likelihood of engaging in automatic gender categorization, suggesting that prior experience played a major role [Gallagher et al., 2024]. Additionally, two studies have shown that categorical perception of gender can be disrupted. Hegarty et al. (year) disrupted categorical perception by presenting faces only in the left visual field. These stimuli were processed by the right brain hemisphere, implying that only the left brain hemisphere displayed categorical perception of gender. The categorical perception effect was also disrupted by the introduction of a third, intermediate response option [Atwood et al., 2024]. These studies together, suggest that the binary view of gender is learned, which implies that it can be unlearned. (this paragraph needs a little more juice)

[Gender categorization research does not account for gender diversity] Moreover, research gender categorization has largely assumed that gender is binary[@hydeFutureSexGender2019, @sapersteinCategoricalGradationalAlternative2021, @lindqvistWhatGenderAnyway2020] often expressed through operationalizations of gender categorization through binary response formats. Two examples of such formats are gender categorization expressed as a single dimension (feminine-masculine) and as binary response options(woman/female and man/male). Both response formats imply ideas about gender: a single dimension suggests that femininity and masculinity are mutually exclusive opposites; and binary response options suggest that those are the only gender categories that exist. Given the findings that gender categorization is susceptible to influence from short-term, methodological manipulations, it is possible that binary response formats bias participants toward a binary conception of gender.

[Gender categorization beyond the binary is becoming a topic of study on its own merits.] Another consequence of this focus on binary gender categorization is that not much is known about categorization of non-binary gender. The increasing prominence and ongoing marginalisation of nonbinary people (Monro, 2019; Jacobsen, 2023) means that this is an important topic of research. For example, there is a misalignment between how many categorize gender, and the existing gender diversity; many people’s tendency to treat gender as both binary and synonymous with sex leads them to misattribute gender (Howansky, 2019). Additionally, it is unclear whether transgender and non-binary people are categorized and stereotyped in the same way, or whether these groups face unique stereotypes. Studies are beginning to answer these questions and more related to categorization of transgender and non-binary gender (see Atwood et al., 2024; van Berlekom et al., 2024; Weissflog et al., 2023 for some recent examples) but this area is still very much nascent. As more research begins to investigate gender categorization beyond the binary, it becomes increasingly important to develop sensitive response formats that accurately represent non-binary gender without implicitly reinforcing gender hierarchies.


## Response Formats Beyond the Binary

*I have collapsed the research of self and other categorization. Why did I do this? Basically because I found that the paper was getting quite repetitious*
Over the years, a number of studies have investigated gender categorization using response formats that challenge gender hierarchies. One solution is to conceptualise femininity and masculinity as independent entities. Unlike one-dimensional treatments of gender, such a two-dimensional approach does not imply that femininity and masculinity are opposites and that an increase in one necessarily entails a decrease in the other. One of the earliest examples of scales to use such an approach was the Bem Sex Role Inventory (BSRI), a questionnaire scale which aimed to measure gender as a psychological trait [@bemMeasurmentPsychologicalAndrogyny1974]. When this scale was used, some participants exhibited clear femininity or masculinity but many scored as "androgynous" (high femininity, high masculinity) and "neutral" (low femininity, low masculinity), destabilising binary notions of gender [@libenUnderstandingUnderminingDevelopment2017]. Such a treatment of gender has been recommended both for participants categorizing their own gender (saperstein) used in face categorization research (Wittlin, 2018).

Another approach has been to broaden the response options when participants are selecting from an established set of categories. @sapersteinCategoricalGradationalAlternative2021 suggested that surveys measuring gender include a range of response options, such as non-binary, other, trans man, agender, and more. This approach is becoming quite common for research that measures participants' categorization of their own gender (li et al., Saperstein et al.). Indeed, variations of this task have also been used to study non-binary categorizations (van Berleko et al., Weissflog et al). One issue with this task is that it does not challenge the idea that gender can be visible from appearance, and may therefore implicitly reinforce the stereotype that non-binary people are androgynous. 

Finally, @lindqvistWhatGenderAnyway2020 suggested that the most sensitive measure for self-categorization would be an open text entry where participants can fill in their gender in an open-ended format. The free text response has the advantage of being completely unconstrained, allowing participants to enter any category, including categories which may not have occurred to the researchers. Moreover, the acceptable terms sometimes shift over time as more marginalised voices are heard. This method has, as far as we are aware, not been tested in the context of categorization of others. 


## The Present Research

Study 1 thested the influence of one and two-dimensional response options on categorical perception. We compare gender categorization measured using one-dimensional response options (ranging from woman to man) and two-dimensional response options (ranging from woman/man to not woman/not man). If categorical perception occurs this would suggest that participants treat gender as consisting of only two categories: women and men. Accordingly, a reduction in this effect would suggest participants take a more fluid view of gender. We tested two research questions:"Do participants respond categorically to faces?" (Research Question 1) and "Does a one-dimensional rating format elicit weaker categorical responses than a two-dimensional rating format (Research Question 2)?"

Study 2 investigated gender categorization using non-binary gender using multiple response options or free text answers. In Study 2 we were mainly interested in how these two measures affect gender categorization in comparisons to only binary response options (Research Question 3). On inspecting the data, we noted a pattern that seemed to indicate that non-binary responses crowded out male responses. Therefore, study 2 also investigated the categorization of women and men (Research Question 4).
...
Swedish participants (*N* = `r length(subs_exp2$id)`) took part in the study at the Stockholm University campus (*M*~age~= `r mean(subs_exp2$age, na.rm = TRUE) %>% round(2)`, *SD*~age~ = `r sd(subs_exp2$age, na.rm = TRUE) %>% round(2)`, Range = `r min(subs_exp2$age, na.rm = TRUE)` - `r max(subs_exp2$age, na.rm = TRUE)`). Self-identified gender was measured using an open-ended text box as recommended by [@lindqvistWhatGenderAnyway2020]. The final sample included 56 women, 47 men, and 2 participants who did not indicate gender. All participants were informed that participation was voluntary and all gave written consent to participate in the study. Participants were randomly allocated to one of the two response option conditions (*N*~binary~ = `r sum(subs_exp2$condition == "xb")`, *N*~multiple~ = `r sum(subs_exp2$condition == "mc")`, *N*~free_text~ = `r sum(subs_exp2$condition == "ft")`). Participants were monetarily compensated for their time (100 sek).
...
The outcome was responses to the categorization task. For analysis purposes, ~~these were recorded in the following ways~~ \[two new variables were created\]{style="color:green";}:

*Other categorizations* represented the trials where participants categorized faces as any other category than woman or man. This was computed by dichotomizing the variable so other was coded as 1 and all other responses were coded as 0. In the free text condition, participants' responses were manually coded so that of other and non-binary counted as other.
...
[Bayesian linear mixed effect models were fit to the data in study 2. These models were the same as in Study 1 apart from the outcome, which was binomial and accordingly had to be modeled as a binomial distribution.]{style="{\"color:green;\""}}
...
An inspection of @fig-desc-1 suggests that participants made fewer man categorizations in the multiple categories condition than the other two conditions. We tested this by examining only responses of woman or man. @fig-wtf illustrates proportions of responses of women and men. Each dot represents a single participant, and the position of the dots on the y axis shows the proportion of faces that participant categorized as man; [the boxplots show median and interquartile range proportion of faces categorized as men]{style="color:green;"} (in this data set, with categorizations beyond the binary removed, any face not categorized as a woman was categorized as a man). Overall rates of binary categorizations were [similar]{style="color:green;"} across the three conditions (see @fig-wtf).
