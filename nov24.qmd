...
abstract: "Psychological research often treats gender as a binary and unidimensional, which ignores the variability of gender and may reinforce a binary view of gender. In two experiments, we studied how response options influence gender categorization of morphed and racially diverse faces. Experiment 1 (N = 71), investigated whether a two-dimensional scale resulted in less binary perception, by comparing uni-and two-dimensional scales in terms of how strongly they elicted categorical perception of gender. Results indicated that gender was perceived as highly categorical, and a two-dimensional scale did not reduce this perception. Experiment 2 (N = 100), compared traditional binary response options with multiple categories and free-text answers in terms of categorization beyond the binary. Results indicated that providing response options than the binary (e.g., “non-binary”, “I don’t know”) made participant perceive gender as more diverse, whereas free-text options elicited largely binary responses. Despite the opportunity to categorize faces beyond the binary, the predominant categorizations remained as ‘woman’ or ‘man’. We conclude that for multiple response options are the best response options for studying gender categorization beyond the binary."
...
Many transgender and gender diverse people experience gender as fluid, diffuse, and outside the typical binary of women and men [@hydeFutureSexGender2019; @richardsNonbinaryGenderqueerGenders2016]. Unlike cisgender people - who identify with their assigned gender at birth - transgender people identify with a gender different from their assigned sex at birth [@levittBeingTransgenderExperience2014]. Moreover, many transgender people identify as nonbinary, which can be either an identity in itself or an umbrella term for a wide variety of gender identities other than woman or man [e.g., genderqueer, agender, genderfluid @monroNonbinaryGenderqueerOverview2019].

Surveys and questionnaires that collect data on gender have typically used response formats that do not account for this gender diversity, for example by including only the categories woman/female and male/man as response options [@sapersteinCategoricalGradationalAlternative2021]. This is beginning to change; it is now more common for studies to include response formats that include options beyond the binary [@sapersteinCategoricalGradationalAlternative2021] or free text answers [@lindqvistWhatGenderAnyway2020]. Research on gender categorization of others, however, is still dominated by binary response options [e.g., @palumboAdaptationComplexPictures2017; @habibiSpontaneousGenderCategorization2012] Moreover, gender categorization may be influenced by the respons options participants use to categorize gender(Atwood et al., 2024; Thorne et al., 2015). The aim of this research is therefore to examine two types of response formats that challenge binary gender: multidimensional scales and multiple response options beyond the binary.

## Gender and Gender Categorization

To understand why response formats matter, it is important to understand the concept of gender itself. Gender is a multifaceted concept with both personal and societal aspects [@hydeFutureSexGender2019]. (*In the previous manuscript we talked about the* social *aspeect of gender)* The personal aspects of gender include a person's physical body (i.e., sex) and their internal sense of gender (i.e., gender identity). Both sex and gender vary on a spectrum; sex is determined by chromosomes, hormones, and physical morphology, none of which need conform to the typical binary and gender can take an encompass an even broader range of expressions and categories. The societal aspects of gender include beliefs about which gender categories exist, stereotypes associated with specific gender categories, as well as the lexicon of words relating to gender (e.g. pronouns, gender labels; Lonergan & Palomares, 2020).

The personal and societal aspects of gender are interconnected [@morgenrothEffectsGenderTrouble2021]; societal norms often constrain the personal aspects of gender, and, inversely, individuals' enactments of gender may influence cultural norms over time [@butler1999]. For example, in many societies that conceive of gender as binary, there are no words or few to express non-binary gender, limiting gender diverse peoples' ability to know and express their identity [@richardsNonbinaryGenderqueerGenders2016a, @thorne2023]. Additionally, the interconnection between the personal and the cultural aspects of gender suggests that inter-individual interactions are important arenas for affirmation or denial of individual's identities. For example, treating a non-binary person as a woman, either by referring to them using feminine pronouns or epecting them to behave in accordance with female stereotypes amounts to a denial of their identity[@juliakapustaMisgenderingItsMoral2016] . People of all gender risk facing such identity denial [@ansaraCisgenderism2016].

Gender categorization is therefore a determining feature of social interactions, as gender categorization activates stereotypes [@freemanDynamicInteractiveTheory2011]. and scripts for behavior (Kessler & McKenna, 1978; Liberman et al., 2017). People base their gender categorizations on others' external features [@yamaguchi, @cloutierInvestigatingEarlyStages2014]) in a process that is effortless [@yangHardDisruptCategorization2019], fast [@tomelleriNatureGenderCategorization2012], and does not require much visual input [@ref]. Indeed, based on this evidence, some have concluded that gender categorization is an automatic [@jungAutomaticityGenderCategorization2019] and unavoidable feature of social interactions (Fiske, 1998).

Moreover, even though faces display a wide variation (Hester et a., 2020) people's gender categorizations of faces exhibit a pattern consistent with categorical perception (Rule et al., 2012; Campanella et al., 2001; Atwood et al., 2024). Categorical perception patterns occur when people consider stimulus that varies on a continuum to belong to just a few categories [@goldstoneCategoricalPerception2010]. In one study, for example, when participants categorized faces that were morphed to vary on a continuum from feminine to masculine, 70% female morphs were categorized women by 90% of participants and a 70% male morphs were categorized as men by 90% percent of participants. This implies that they treated gender as a binary consisting of women and men only. Though it should be noted that most studies on categorical perception have used a binary response paradigm, where man an woman are the only options.

Recent work suggests that a binary view of gender is not inevitable. @thorneLeftHemisphereAndrocentric2015 found that participants only displayed categorical perception when face were presented in the right visual field (i.e. when processed by the left brain hemisphere). In contrast, faces presented in the left visual field (i.e. processed by the right brain hemisphere) did not elicit categorical perception. Moreover, Atwood and colleagues (2024) tested categorical perception in a discrimination task. Participants were unlikely mix up slightly feminine and mascline faces when woman and man were the only options, but equally likely to mix them up when a third, non-binary option was included. The study suggests that response formats in particular play a role in determining partiipants conceptions of gender.

## Response Formats and Gender Categorization Beyond the Binary

Research gender on categorization has largely operationalized gender using response formats that suggest that gender is binary [@hydeFutureSexGender2019, @sapersteinCategoricalGradationalAlternative2021, @lindqvistWhatGenderAnyway2020]. Two examples of such formats are gender categorization expressed as a single dimension (feminine-masculine) and as binary response options(woman/female and man/male). Both response formats imply ideas about gender: a single dimension suggests that femininity and masculinity are mutually exclusive oppositesl and binary response options suggest that those are the only gender categories that exist. It is therefore possible that binary response formats bias participants toward a binary conception of gender.

As gender is diverse, it is important to understand which types of response formats capture gender diversity. One approach is to operationalize femininity and masculinity as independent dimensions. Unlike one-dimensional treatments of gender, a two-dimensional approach does not imply that femininity and masculinity are opposites and that an increase in one necessarily entails a decrease in the other, destabilizing binary notions of gender [@libenUnderstandingUnderminingDevelopment2017]. Two-dimensional approaches have been used to assess gender as a psychological trait (Bem, 1974), as self-idendified categories [@sapersteinCategoricalGradationalAlternative2021] and as categorization of others [@wittlinFaceMemoryTransgender2018]. Moreover, at least one study found that femininity and masculinity were independent dimensions [@hesterPerceivedFemininityMasculinity].
